                ...
                <div class="table-responsive">
                    <table class="categories-show-table table table-hover table-striped table-bordered" >
                        ...
                        <tr>
                            ...
                            @if(adminAccessRoute(array_merge(config('role.remit_operation.access.edit'), config('role.remit_operation.access.delete'))))
                        ...
                        <tbody>
                            <tr v-for="(item , index ) in items">
                                ...
                                <td data-label="@lang('Name')">@{{item.title}}</td>
                                ...
                                <td data-label="@lang('Action')">
                                    ...
                                    @endif
                                    @if(adminAccessRoute(config('role.remit_operation.access.delete')))
                                    <a href="javascript:void(0)"
...
@push('js')
    ...
    <script>
        ...
        var app = new Vue({
            ...
            methods: {
                getItems() {
                    axios.get("{{route('admin.list.sourceOfFund')}}")
                        .then(response => {
                ...
                createItem() {
                    ...
                    var _this = this;
                    axios.post("{{route('admin.store.sourceOfFund')}}", this.newItem)
                        .then(response => {
                ...
                setVal(items) {
                    ...
                    this.oldItem.title = items.title;
                },
                updateItem() {
                    var _this = this;

                    axios.post("{{route('admin.update.sourceOfFund')}}", this.oldItem)
                        .then(response => {
                            var data = response.data;
                            if (data.status === 'success') {
                ...
                deleteItem(id) {
                    ...
                    axios.post("{{route('admin.delete.sourceOfFund')}}", social)
                        .then(res => {
