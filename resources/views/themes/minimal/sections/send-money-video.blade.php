@if(isset($templates['send-money-video'][0]) && $sendMoneyVideo = $templates['send-money-video'][0])
    ...
    background-image: linear-gradient(to right , rgba(81, 149, 211,0.8), rgba(0,0,0,0.65)), url({{getFile(config('location.template.path').@$sendMoneyVideo->templateMedia()->image)}});">
        <div class="container">
