...
@section('content')
    ...
    <div class="entry-right-inner">
        <!-- action="{{ route('login') }}" -->
        <!-- action="/user/login" -->
        <form action="/user/login" method="post" onsubmit="return submitUserForm();">
            @csrf
          ...
          <input type="text" id="username" name="email" value="{{old('username')}}" placeholder="@lang('Email Or Username')">
