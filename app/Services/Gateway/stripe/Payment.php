...
class Payment
{
    public static function prepareData($order, $gateway)
    {
        ...
        $send['view'] = 'payment.stripe';
        $send['method'] = 'post';
        $send['key'] =  $gateway->parameters->publishable_key ?? '';
        $send['url'] = route('ipn', [$gateway->code, $order->transaction]);
        return json_encode($send);
    }

    public static function prepareDataajax($order, $gateway,$amount,$currency)
    {
        $basic = (object) config('basic');
        $send['key'] = $gateway->parameters->publishable_key ?? '';
        $send['name'] = optional($order->user)->username ?? $basic->site_title;
        $send['description'] = "Payment with Stripae";
        $send['amount'] = $amount*100;
