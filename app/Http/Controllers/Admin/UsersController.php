...
class UsersController extends Controller
{
    ...
    public function userUpdate(Request $request, $id)
    {
        ...
        $user->username = $userData['username'];
        $user->email = $userData['email'];
        $user->phone = "+".$userData['phone_code'].$userData['phone'];
        ...
        $user->status = ($userData['status'] == 'on') ? 0 : 1;
        if(isset($userData['email_verification']))
        $user->email_verification = ($userData['email_verification'] == 'on') ? 0 : 1;
        if(isset($userData['sms_verification']))
        $user->sms_verification = ($userData['sms_verification'] == 'on') ? 0 : 1;
        if(isset($userData['two_fa_verify']))
        $user->two_fa_verify = ($userData['two_fa_verify'] == 'on') ? 1 : 0;
        if(isset($userData['two_fa_auth']))
    ...
    {
        $logs = UserLogin::with('user')->orderBy('id', 'DESC')->paginate(config('basic.paginate'));
