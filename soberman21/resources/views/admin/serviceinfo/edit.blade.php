                    ...
                    <div class="card-body">
						...
						<div class="tab-content mt-2" id="myTabContent">
                        	...
                        	<div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" id="lang-tab-{{ $key }}" role="tabpanel">
                        	  @if($key == 0)
		                        <form method="post" action="{{route('admin.serviceinfo.update',$method)}}"
		                      ...
		                      @else
		                      @php $child_category = \App\Models\Serviceinfo::where(['parent_id' => $method->id,'language_id' => $language->id])->first();  @endphp
		                      <form method="post" action="{{route('admin.serviceinfo.update',$method)}}"
		                              class="needs-validation base-form" novalidate="" enctype="multipart/form-data">
		                              <input type="hidden" name="language_id" value="{{$language->id}}"/>
		                              <input type="hidden" name="service_id" value="{{$method->service_id}}"/>
