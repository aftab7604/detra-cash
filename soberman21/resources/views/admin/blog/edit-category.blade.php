            ...
            <div class="col-12">
                <div class="card card-primary shadow">
                    ...
                    <div class="card-body">
                        ...
                        <a href="{{route('admin.blog.category')}}" class="btn btn-success btn-sm float-right mb-3"><i class="fa fa-list"></i> {{trans('Blog Category')}}</a>
                        ...
                        <div class="tab-content mt-2" id="myTabContent">
                        	...
                        	<div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" id="lang-tab-{{ $key }}" role="tabpanel">
		                      @if($key == 0)
		                        <form method="post" action="{{route('admin.update.blog.category',$method)}}"
		                        ...
		                        @else
		                        @php $child_category = \App\Models\BlogCategory::where(['parent_id' => $method->id,'language_id' => $language->id])->first();  @endphp
		                        <form method="post" action="{{route('admin.update.blog.category',$method)}}"
		                              class="needs-validation base-form" novalidate="" enctype="multipart/form-data">
