    ...
    <div class="container-fluid">
        @if($payout)
            <div class="bd-callout bd-callout-warning  ">
                <i class="fas fa-info-circle mr-2"></i>  {{ $payout->last_error }}
            </div>
        @endif
                ...
                <div class="card shadow">
                    <div class="card-body">
                        ...
                        <div class="d-flex justify-content-between align-items-center">
                            <h4 class="card-title">@lang("Withdraw Details")</h4>

                            <div>
                                @if($payout->status == 1)
                                    <a href="{{route('admin.withdraw-confirm',$payout->id)}}"
                                    ...
                                    <a href="{{route('admin.withdraw-cancel',$payout->id)}}"
                                       data-toggle="modal"
                                       data-target="#confirmModal"
                                       class="btn btn-danger btn-sm confirmButton"><i class="far fa-times-circle"></i> @lang('Reject')</a>
                                @endif

                                <a href="{{route('admin.withdraw-history')}}" class="btn btn-sm btn-primary ml-2">
                                    ...
                                    <ul class="list-style-none">
                                        ...
                                        <li class="my-3">
                                            <span class="font-weight-bold text-dark"><i
                                                    ...
                                                    class="font-weight-medium text-info">{{ __(optional($payout->payoutMethod)->name) }}</span></span>
                                        </li>
                                        ...
                                        <li class="my-3">
                                            <span><i
                                                    ...
                                                    {{ (getAmount($payout->amount+$payout->charge,2)).' '.config('basic.currency') }}
                                                </span>
                                            </span>
                                        </li>
                                    </ul>
                                    ...
                                    @if(isset($payout->remarks))
                                        <ul class="list-style-none border-bottom">
                                            ...
                                            <span><i class="icon-check mr-2 text-primary"></i> @lang('Bank Currency') : <span
                                                    class="font-weight-bold text-danger">{{ __($payout->currency) }}
                                                </span>
                                            </span>
                                            </li>


                                            @foreach(json_decode($payout->remarks) as $key => $value)
                                            <!-- {!! $value->name !!} -->
                                                <li class="my-3">
                                                    <span><i class="icon-check mr-2 text-primary"></i> {{ __(@$value->fname) }} :
