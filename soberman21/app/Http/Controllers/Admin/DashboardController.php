...
class DashboardController extends Controller
{
    ...
    {
        $data['reloadlyTrx'] = collect(TopUpLog::selectRaw('COUNT(CASE WHEN status = 1 THEN id END) AS success')
            ->selectRaw('COUNT(CASE WHEN status = 0 THEN id END) AS failed')
            ->get()->toArray())->collapse();
        $reloadlyAccount = ReloadlyService::accountBalance();
		
		if(isset($reloadlyAccount->errorCode) && $reloadlyAccount->errorCode == 'INVALID_TOKEN'){
			return redirect()->route('admin.basic-controls')->with('error', "Reloadly: ". $reloadlyAccount->message);
		}
		
		if(isset($reloadlyAccount->errorCode) && $reloadlyAccount->errorCode == 'TOKEN_EXPIRED'){
			 $token =  ReloadlyService::accessToken();
			$reloadlyAccount = ReloadlyService::accountBalance();
		}
	
		
		$data['reloadlyAccount'] = @$reloadlyAccount;
    ...
    public function passwordUpdate(Request $request)
    {
        ...
        log_admin_activity('Update password', 'Changed thire password of the acount');
